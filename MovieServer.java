...
public class MovieServer {
    ...
    private static final List<Movie> movies = Collections.synchronizedList(new ArrayList<>());
        ...
        for (Movie m : movies) {
            if (m.getId() == id) {
                if (m.isAvailable()) {
                    ...
                    return "SUCESSO: Filme '" + m.getName() + "' alugado com sucesso!";
                }
    ...
    public static synchronized String returnMovie(int id) {
        for (Movie m : movies) {
            if (m.getId() == id) {
                if (!m.isAvailable()) {
                    m.setAvailable(true);
                    return "SUCESSO: Filme '" + m.getName() + "' devolvido com sucesso!";
                }
